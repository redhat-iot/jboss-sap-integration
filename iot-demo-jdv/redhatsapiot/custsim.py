...
class Location:
   def __init__(self,x,y,width, height):
      ...
      self.validMoves = [(a,b, True if a <= self.x and b <= self.y else False) for a in range(max(self.x - 1, 0), min(self.x + 20,width)) for b in range(max(self.y -1, 0), min(self.y + 20, height)) if not(a == self.x and b == self.y)]
   ...
   def manageCustomerMovements(c):
      if c.tick():
         if c.isExiting and c.currentLocation.x == 0 and c.currentLocation.y == 0:
            ...
            print('%s is Exiting: %s') % (c.name, msg)
            ...
            print('%s is Moving: %s') % (c.name, msg) 
